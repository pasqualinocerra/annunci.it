        ...
        <div class="container">
            ...
            <form action="{{ route('annunci.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group mt-3">
                    <label for="titolo" class="mt-2">Titolo annuncio</label>
                    <input type="text" class="form-control mt-3" id="titolo" name="titolo" placeholder="Inserisci il titolo dell'annuncio">

                    <label for="descrizione" class="mt-2">Descrizione dell'annuncio</label>
                    <input type="text" class="form-control mt-3" id="descrizione" name="descrizione" placeholder="Inserisci la descrizione">

                    <label for="prezzo" class="mt-2">Prezzo</label>
                    <input type="text" class="form-control mt-3" id="prezzo" name="prezzo" placeholder="Inserisci il prezzo">

                    <label for="categoria" class="mt-2">Selezione una categoria:</label>
                    <select class="form-control" name="categoria" id="categoria">
                        <option value="">-- Selezione una categoria --</option>
                        @foreach ($categorie as $categoria )
                            <option value="{{ $categoria->id }}" class="mt-1">{{ $categoria->descrizione }}</option>
                        @endforeach
                    </select>
                    <label for="image" class="mt-2">Carica immagine:</label>
                    <input type="file" class="form-control mt-1" name="image" id="image" required>

                    <button type="submit" class="btn btn-success mt-3">Salva annuncio</button>
                </div> 
            </form>
            @if (session('success'))
