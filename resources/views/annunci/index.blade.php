...
<body>
    <nav class="navbar navbar-expand-lg bg-body-tertiary">
        <div class="container-fluid">
            <a class="navbar-brand" href="/"><img src="{{ asset('annunci.png') }}" width="200px" height="80px"></a>
    ...
    <main>
        ...
        <div class="container">
            @if (session('pubblicato_SN') == 'ok')
                <div class="alert alert-success mt-3">
                    ANNUNCIO PUBBLICATO!
                </div>
            @endif
            <div class="row row-cols-1 row-cols-md-3 g-4">
                @foreach ($annunciDaRevisionare as $annuncioDaRevisionare)
                    <div class="col">
                        <div class="card">
                            <img src="{{ asset('xbox.jpg') }}" class="card-img-top" alt="">
                            <div class="card-body">
                                <h5 class="card-title">{{$annuncioDaRevisionare->titolo}}</h5>
                                <p class="card-text">{{$annuncioDaRevisionare->descrizione}}</p>
                                <p class="card-text">{{$annuncioDaRevisionare->prezzo}} euro</p>
                            </div>
                        </div>
                        <div class="mt-1">
                            <form action="{{ route('pubblica.annuncio', [$annuncioDaRevisionare->id])}}" method="post">
                                @csrf
                                <button type="submit" class="btn btn-danger">Pubblica annuncio</button>
                            </form>
                        </div>
                    </div>
                @endforeach
